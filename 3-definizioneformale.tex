...
\begin{equation*}
	Mh(c)\in \{16,24,48\} \quad \forall c\in C
\end{equation*}
...
\begin{equation}
	riposo(c,d) \ge 1-0.5*\sum_{s\in S_d}x_{c,s}, \quad \forall c\in FT\cup PT, \forall d\in D
\end{equation}
...
\begin{equation}
	O(c) \ge \sum_{s \in S} x(c,s)*dur(s)- Mh(c) \quad \forall c \in C\label{vincolo:straordinari}
\end{equation}
...
\begin{equation}
	\Delta(d,k) = \sum_{c \in C}\sum_{s \in S(d)} copre(s,d,k)x(c,s) - f(d,k)  \quad \forall d,k\\
\end{equation}
...
\begin{align}
	& \Delta(d,k) = \sum_{c \in C}\sum_{s \in S(d)} pesoCopertura(c)*copre(s,d,k)x_{w,s} - f(d,k)
	&& \forall d,k\\
