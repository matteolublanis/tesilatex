...
Si indica con $w$ un dipendente generico appartenente al roster. I dipendenti si differenziano tra loro principalmente per gli orari contrattuali assegnali, introduciamo quindi gli insiemi:
\begin{center}
	$W=FT\cup PT \cup PTW$\\
...
\end{align}
La copertura del fabbisogno si specifica mediante il primo vincolo. Questo vincolo assicura che almeno $f_{d,k}$ lavoratori siano presenti nello slot temporale $k$ del giorno $d$. Con questo vincolo non è ammissibile una situazione di difetto di personale, ma è ammissibile un eccesso di personale. Si potrebbe ammettere una situazione di difetto modificando il valore di $f_{d,k}$ o cambiando il peso del vincolo (introducendo ad esempio una nuova variabile $p_f$, il cui valore può ridurre o aumentare $f_{d,k}$). 
