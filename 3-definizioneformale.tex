...
Durante una giornata, l'obiettivo del caporeparto è quello di coprire il flusso della clientela in maniera adeguata. Rifacendosi al problema del \textit{Set Covering}, all'articolo di Brucker\cite{BRUCKER2011467} e a quello di Gusmeroli e Bettinelli\cite{gusmeroli2024mixedintegerlinearprogramcreate}, si definiscono:
\begin{center}
...
\begin{itemize}
	...
	\item end: slot di fine del turno (inclusivo, indica l'ultimo slot lavorato).
...
\begin{displayquote}
	\textit{Part time weekend: 20 ore settimanali, con disponibilità limitata ai giorni di venerdì (solo il turno di chiusura), sabato e domenica.\dots\\
...
\begin{align}
	...
	&maxTurni(c,d) =
	\begin{cases}
		2 & \text{se } c\in FT \lor (c\in PTW \land d\in {6,7})\\
		1 & \text{se } c\in PT \lor (c\in PTW \land d\in {5} \land k\ge k(16:30)) \\
		0 & \text{altrimenti}
	\end{cases}\notag
\end{align}
...
\end{align}
La copertura non può essere sempre coperta rigidamente, in quanto il personale può non essere adeguato all'effettiva richiesta in alcuni situazioni. Considerato questo, si introducono due nuove variabili:
\begin{align}
	&cover(d,k)=\sum_{S(d)}\sum_{c\in C}x(c,s)copre(s,d,k)\\
	&u(d,k)\ge 0\\
	&cover(d,k), u(d,k) \in \mathbb{N}
\end{align}
con le quali si garantisce la copertura mediante il seguente vincolo:
\begin{align}
	&cover(d,k)+u(d,k)\ge f(d,k) 
	&&\forall d\in D, \forall k\in T(d)\label{coperturafabbisogno}
\end{align}
La copertura del fabbisogno rappresenta l'obiettivo principale dell'intero problema, per cui:
\begin{align}
	&f_1=\sum_{d,k}u(d,k)
\end{align}
andrà minimizzato.
Si vuole anche ridurre le ore complessive lavorate dai cassieri:
\begin{equation}
	f_2=\sum_{c \in C}H_{settimana}(c)
\end{equation}
...
\begin{align}
	...
	&&\forall c\in FT\cup PTW, \forall d
\end{align}
In quanto si è assicurato della non sovrapposizione, possiamo definire:
\begin{equation}
...
\begin{equation}
	f_3=\sum_{d \in D}\sum_{c \in FT\cup PTW} stacco(c,d)
\end{equation}
...
\begin{equation}
	f_6=\sum_{s \in S, c\in C}pen(s)x(c,s) 
\end{equation}
...
\begin{equation}
	f_7=\sum_{c\in (FT\cup PT)}riposo(c,7)
\end{equation}
...
\begin{equation}
	f_8=\sum_{c\in (FT)}\sum_{d\in {1,\dots,m-1}}riposoContinuato(c,d)
\end{equation}

\paragraph{Cassa-reparto}
I vincoli soft relativi al fabbisogno non sfavoriscono la situazione generale di deviazione dal fabbisogno dichiarato: questo porta a situazioni di sottocopertura sfavorite di molto rispetto a quelle di sovracopertura, che non vengono in alcun modo influenzate dai vincoli. Si introducono:
\begin{align}
	&under(d,k)\in \mathbb{R}{\ge 0} \qquad \text{(carenza)}\\
	&over(d,k)\in \mathbb{R}{\ge 0} \qquad \text{(sovracopertura)}
\end{align}
dove $under(d,k)$ indica di quanto si è sotto il fabbisogno nello slot $(d,k)$, mentre $over(d,k)$ indica di quanto si è sopra.

La copertura effettiva dello slot $(d,k)$ è data dalla somma dei turni assegnati che coprono lo slot, pesati in base alla skill cassa-reparto:
\begin{align}
	& \text{cop}(d,k)= \sum_{c \in C}\sum_{s \in S(d)} pesoCopertura(c)*copre(s,d,k)*x(c,s)
\end{align}
La variabile introdotta è molto simile a $cover(d,k)$, ma introduce un nuovo valore che è $pesoCopertura(c)$, necessario per esprimere formalmente la skill di cassa-reparto. Un cassiere che possiede la skill cassa-reparto ha un peso minore nel conteggio della sovracopertura, in quanto può uscire in corsia in caso di eccesso di cassieri in cassa, riducendo il carico sul reparto.
...
La deviazione assoluta dal fabbisogno viene modellata imponendo:
\begin{align}
	& \text{cop}(d,k) + U(d,k) - O(d,k) = f(d,k)
	&& \forall d\in D,; k\in T(d)
\end{align}

In questo modo:
\begin{itemize}
	\item $\text{cop}(d,k) < f(d,k)$, $U(d,k)>0$ (carenza);
	\item $\text{cop}(d,k) > f(d,k)$, $O(d,k)>0$ (sovracopertura);
	\item $\text{cop}(d,k) = f(d,k)$, $U(d,k)=O(d,k)=0$.
\end{itemize}

L’obiettivo da minimizzare è il seguente:
\begin{equation}
	f_9=\sum_{d\in D}\sum_{k\in T(d)} (U(d,k) + O(d,k))
\end{equation}
Questa funzione obiettivo può essere divisa in due per riuscire a gestire situazioni di carenza e sovracopertura in maniera differente mediante l'uso dei pesi degli obiettivi:
\begin{align}
	&f_{9under}=\sum_{d\in D}\sum_{k\in T(d)} U(d,k)\\
	&f_{9over}=\sum_{d\in D}\sum_{k\in T(d)} O(d,k)
\end{align}
...
\end{align}
In quanto la finestra temporale è limitata ad una singola settimana, il problema del controllo del rispetto del vincolo delle 11 ore di riposo tra la domenica e il lunedì può essere spostato dalla definizione del modello al problema dell'assegnamento dei turni.

\subsection{Funzione obiettivo}
\begin{align}
    \min \; & 
    \sum_{i=1}^9\lambda_if_i
    && \lambda_i \in \mathbb{R}
\end{align}
I $\lambda_i$ rappresentano i pesi che noi diamo ai vari obiettivi: più $\lambda_i$ è elevato, più l'obiettivo relativo incide nella funzione obiettivo generale, ottenendo di fatto più importanza nella ricerca dell'ottimo. $\lambda_i$ può essere negativo, in modo tale da massimizzare il valore del relativo obiettivo.
